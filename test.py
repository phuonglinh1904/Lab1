...
from gradient_descent import grad_desc
...
x_0, y_0 = 1, 1
epsilon = 0.0001
points = grad_desc(x_0=x_0, y_0=y_0, learning_rate=0.01, epsilon=epsilon)
ax = plt.subplots()[1]
horizontal_axis = np.array(points[0:len(points):2])
vertical_axis = np.array(points[1:len(points):2])
plt.scatter(horizontal_axis, vertical_axis, marker='*', color='red')
ax.plot(x,y)
ax.contour(X, Y, F, 50)
plt.show()
